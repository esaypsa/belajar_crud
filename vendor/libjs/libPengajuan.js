...
var tbPengajuan= $('#table_pengajuan');
  ...
  ajax: {
     ...
     processResults: function (data, page) {
       return {
        ...
        initTable(tbPengajuan);
        var id = $(".select2 option:selected").val();
        LoadDataUser(id);

});

document.getElementById("add_btn").innerHTML = "<i class='bx bx-customize'></i> Tambah Pengajuan"
...
$('#select_tujuan').select2({
  ...
  ajax: {
     dataType   : 'json',
     url        : site_url+class_url+'/select_tujuan',
     type       :'post',
     delay      : 800,
     data       : function(params) {
                 return {
...
function loadStatusPengajuan(){
  $('#id_pengajuan').load(site_url+class_url+'/select_pengajuan');
}
...
function loadStatus(){
  $('#jenjang').load(site_url+class_url+'/select_status');
}
function loadGolongan(){
  $('#jenjang').load(site_url+class_url+'/select_golongan');
}
